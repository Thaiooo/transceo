    	...
    	<g:else>
	    	<table class="spip" width="100%">
    			<thead>
	    			<tr class="row_first">
		    			<th></th>
		    			<g:sortableColumn action="sortReservationToProcess" property="id" title="${message(code:'label.travel.id')}" />
		    			<g:sortableColumn action="sortReservationToProcess" property="creationDate" title="${message(code:'label.travel.creation.date')}" />
		    			<g:sortableColumn action="sortReservationToProcess" property="customer" title="${message(code:'label.travel.member')}" />
		    			<g:sortableColumn action="sortReservationToProcess" property="travelDate" title="${message(code:'label.travel.traveldate')}" />
		    			<th><g:message code="label.travel.depart" /></th>
		    			<th><g:message code="label.travel.destination" /></th>
		    			<g:sortableColumn action="sortReservationToProcess" property="numberOfPersonne" title="${message(code:'label.travel.number.personne')}" />
		    		</tr>
	    		</thead>
	    		<tbody>
			    	<g:each status="i" in="${travels}" var="item">
						<tr class="${ (i % 2) == 0 ? 'row_even' : 'row_odd'}">
			     			<td><g:link controller="administrator" action="showForValidateReservation" id="${item.id}"><g:message code="label.travel.action.administrate" /></g:link></td>
			     			<td>${item.id}</td>
			     			<td>
