...
alter table special_condition drop foreign key FKBAE8D515DE24C296;
...
create table special_condition (id bigint not null auto_increment, version bigint not null, member_id bigint not null, start_date datetime not null, value double precision not null, label varchar(255) not null, end_date datetime, type varchar(255) not null, primary key (id));
...
alter table special_condition add index FKBAE8D515DE24C296 (member_id), add constraint FKBAE8D515DE24C296 foreign key (member_id) references member (id);
