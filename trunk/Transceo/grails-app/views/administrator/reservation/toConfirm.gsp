    	...
    	<g:else>
	    	<table class="spip" width="100%">
    			<thead>
	    			<tr class="row_first">
		    			<th></th>
		    			<g:sortableColumn action="sortReservationToConfirm" property="id" title="${message(code:'view.travel.id')}" />
	    		...
	    		<tbody>
	    			<g:each status="i" in="${travels}" var="item">
						<tr class="${ (i % 2) == 0 ? 'row_even' : 'row_odd'}">
			     			<td><g:link controller="administrator" action="showForConfirmReservation" id="${item.id}"><g:message code="view.travel.action.administrate" /></g:link></td>
			     			<td>${item.id}</td>
			     			<td>
